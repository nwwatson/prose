...
### Content Import (WordPress)
Admins upload a WordPress WXR file at `/admin/imports` (`Admin::ImportsController`, admin role only). `Import` (`source` enum — only `wordpress` today, `validate: true`; `status` pending/processing/completed/failed; `has_one_attached :file` validated as XML ≤ `Import::MAX_FILE_SIZE` (50 MB); `stats` JSON) is processed by `ImportJob`, which opens the stored file, runs `Import#importer` (`Import.importer_for(source)`), saves the returned stats, and broadcasts the `admin/imports/_import` partial on the `"imports"` stream. Add Ghost/Substack by adding a `source` enum value and a branch in `Import.importer_for`.

`Imports::WordpressImporter.new(io:, user:, downloader:)#call` → stats hash (`posts_imported`, `pages_imported`, `skipped`, `failed`, `media_downloaded`, `media_failed`, `warnings` capped at `Import::MAX_WARNINGS`, `warnings_truncated`). All content is assigned to the importing admin. Status mapping: `publish`→published, `future`→scheduled (published if the date has passed), `draft`/`pending`/`private`→draft, anything else (trash, auto-draft) skipped. First non-"uncategorized" category → `category`; remaining categories + tags → `tags` (matched by slug, then name). Excerpt → `meta_description`. Items whose slug already exists (and pages with `Page::RESERVED_SLUGS`) are skipped, making re-runs idempotent. Each item runs inside `import_item`, which confines an unexpected `StandardError` (e.g. odd markup crashing the converter) to that item — it is logged, recorded as a warning and counted in `failed`, and the import continues; failures reading the export itself happen outside it and still fail the whole import. **Each item is saved exactly once with content/featured image/taxonomy already set** — that is what keeps imports silent: notifications only fire from `Post#publish!`, and `Post::Webhookable`/`Versionable` only react to updates. Don't split an imported post into create-then-update. Media is downloaded *before* the per-item transaction so SQLite's write lock isn't held across HTTP; downloaded blobs left unattached at the end are purged.

`Imports::Wordpress::WxrParser` reads namespace URIs from the document (WXR 1.0–1.2 differ; `remove_namespaces!` would collapse `content:encoded`/`excerpt:encoded`), parses with `nonet` (no external entities), and resolves `_thumbnail_id` postmeta to attachment URLs. `Imports::Wordpress::ContentConverter` strips Gutenberg `<!-- wp:* -->` comments, runs a `wpautop` port only on non-block (classic) content, converts `[caption]` → figure, `[embed]`/`[youtube]` and YouTube/Vimeo iframes → links, removes `[gallery]`/`[audio]`/`[video]`/`[playlist]` with a warning, **leaves unknown shortcodes as text** (with a warning) so bracketed prose isn't lost, strips scripts/styles/event-handler attributes, and replaces each `<img>` (preferring a linked full-size file, using `<figcaption>` as caption) with an `ActionText::Attachment`; failed downloads keep the remote `<img>`. `Imports::MediaDownloader` is the SSRF-safe fetcher: every URL and redirect hop (max 3) goes through `Webhooks::UrlGuard.resolve!` with the connection pinned via `Net::HTTP#ipaddr=`, bodies are capped at 10 MB while streaming, both the header and the sniffed (Marcel) type must be `image/*`, results are memoized per URL, and it never raises (failures are recorded in `#failures`). Tests stub `Net::HTTP.new` and `Webhooks::UrlGuard.resolve` rather than hitting the network; the fixture export is `test/fixtures/files/wordpress.xml`.
