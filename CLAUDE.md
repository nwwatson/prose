# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project Overview

Prose is a Ruby on Rails 8.1 application (Ruby 3.4.4) using the "Solid" stack — SQLite3 for all persistence with Solid Queue, Solid Cache, and Solid Cable for jobs, caching, and WebSockets respectively. No external services (Redis, etc.) are required.

Frontend uses Hotwire (Turbo + Stimulus), Tailwind CSS, Propshaft asset pipeline, and ImportMap for JavaScript modules.

## Common Commands

### Development
```bash
bin/setup              # Full dev environment setup (idempotent), starts server
bin/setup --skip-server # Setup without starting server
bin/setup --reset      # Reset database during setup
bin/dev                # Start dev server (Puma + Tailwind watcher on port 3000)
```

### Testing
```bash
bin/rails test                        # Run all unit tests
bin/rails test test/models/user_test.rb  # Run a single test file
bin/rails test test/models/user_test.rb:15  # Run a single test by line number
bin/rails test:system                 # Run system (browser) tests
bin/rails db:test:prepare             # Prepare test database
```

### CI Pipeline
```bash
bin/ci                 # Run full CI locally (setup, lint, security, tests)
bin/rubocop            # Ruby linting (rubocop-rails-omakase style)
bin/brakeman --quiet --no-pager --exit-on-warn --exit-on-error  # Security scan
bin/bundler-audit      # Gem vulnerability audit
bin/importmap audit    # JS dependency audit
```

### Database
```bash
bin/rails db:prepare   # Create and migrate database
bin/rails db:migrate   # Run pending migrations
bin/rails db:seed      # Load seed data
bin/rails db:reset     # Drop and recreate from schema
```

The app uses `config.active_record.schema_format = :sql`, so **`db/structure.sql` is the
authoritative schema** — SQL format is required to preserve the FTS5 virtual tables and
triggers that Rails' Ruby schema dumper cannot express. There is no `db/schema.rb`; it is
gitignored, and `db/*_schema.rb` (the generated Solid Queue/Cache/Cable dumps) plus
`db/schema.rb` are excluded from RuboCop in `.rubocop.yml`.

## Architecture

Refer to `docs/design_guide.md` for comprehensive architectural patterns. Key principles:

### Model Organization
Models use **concerns for behavior composition**. Each concern lives in a directory matching the model name:

```
app/models/user.rb                    # class User includes concerns
app/models/user/named.rb              # module User::Named (concern)
app/models/user/authenticatable.rb    # module User::Authenticatable (concern)
app/models/user/api_tokenable.rb     # module User::ApiTokenable (concern)
app/models/user/passkey_authenticatable.rb # module User::PasskeyAuthenticatable (WebAuthn passkeys)
app/models/post/discoverable.rb      # module Post::Discoverable (related posts, prev/next)
app/models/post/versionable.rb      # module Post::Versionable (revision history, version cooldown)
app/models/post/webhookable.rb      # module Post::Webhookable (fires post.published/updated/deleted/scheduled/unpublished webhook events)
app/models/webhook.rb               # Webhook: outbound webhook subscription (url, events, encrypted signing_secret, auto-disable)
app/models/post/federatable.rb      # module Post::Federatable (fediverse Create/Update/Delete on publish/edit/unpublish/destroy)
app/models/site_setting/activity_pub_configuration.rb # module SiteSetting::ActivityPubConfiguration (federation toggle, username, RSA keypair)
app/models/fediverse_actor.rb       # FediverseActor: cached remote ActivityPub actor (inbox, public key); followed_at marks a follower
app/models/fediverse_like.rb        # FediverseLike: a remote actor's Like of a post
app/models/webhook_delivery.rb      # WebhookDelivery: per-attempt delivery log for a Webhook (pruned to Webhook::DELIVERY_LOG_LIMIT)
app/models/post_version.rb          # PostVersion: full snapshot of post content per version
app/models/reading_list_item.rb     # ReadingListItem: identity ↔ post bookmark (unique per pair, capped at MAX_ITEMS = 500 per identity)
app/models/identity/reading_listable.rb # module Identity::ReadingListable (bookmark!/unbookmark!, reading_list_posts/_post_ids, import_reading_list!)
app/models/concerns/sluggable.rb     # module Sluggable (slugged_from macro: slug generation/uniquifying, used by Post, Page, Category, Tag)
app/models/concerns/identity_backed.rb # module IdentityBacked (belongs_to :identity, email validation/normalization, build_identity_if_needed; overridable default_identity_name), included by User and Subscriber
app/models/comment/editable.rb        # module Comment::Editable (15-min edit window, soft delete)
app/models/comment/notifiable.rb      # module Comment::Notifiable (reply notification callbacks)
app/models/page.rb                    # class Page (custom static pages)
app/models/navigation_item.rb        # NavigationItem: header/footer/social menu links (label, url, location enum, position, open_in_new_tab)
app/models/navigation_item/cacheable.rb # module NavigationItem::Cacheable (for_location cached per location, expired after_commit)
app/models/navigation_item/social_platform.rb # module NavigationItem::SocialPlatform (infers icon platform from URL host)
app/models/concerns/publishable.rb   # module Publishable — shared `publishes_at` macro (live scope, publish!/schedule!/revert_to_draft!), included by Post, Page, Newsletter
app/validators/future_validator.rb   # FutureValidator: shared "must be in the future" validation used by Publishable
app/validators/hex_color_validator.rb # HexColorValidator: shared `validates :x, hex_color: true` for #RRGGBB colors, used by SiteSetting::DarkTheme, SiteSetting::EmailBranding, SubscriberLabel, Newsletter::Templatable
app/models/currency.rb               # Currency: SYMBOLS/NAMES/CODES, `.symbol`, `.format(cents, code)`, `.options_for_select` — used by MembershipTier#formatted_price and SiteSetting::PaymentConfiguration#currency_symbol/SUPPORTED_CURRENCIES
app/models/site_setting/localization.rb  # module SiteSetting::Localization (i18n)
app/models/identity/handleable.rb    # module Identity::Handleable (handle validation/normalization)
app/models/identity/profileable.rb   # module Identity::Profileable (avatar, bio, social links)
app/models/api_token.rb              # Token generation, digest lookup, revocation
app/models/passkey.rb                # WebAuthn passkey credentials for admin sign-in
app/models/subscriber_label.rb       # Labels for subscriber segmentation (name, color)
app/models/subscriber_labeling.rb    # Join model: subscribers ↔ subscriber_labels
app/models/segment.rb                # Saved subscriber filters (JSON filter_criteria)
app/models/segment/resolvable.rb     # module Segment::Resolvable (resolves criteria to subscribers)
app/models/segment/criteria_builder.rb # module Segment::CriteriaBuilder (flat form attrs ↔ filter_criteria JSON)
app/models/membership_tier.rb       # MembershipTier: paid membership plans (name, price, interval)
app/models/membership_tier/syncable.rb # module MembershipTier::Syncable (syncs to Stripe products/prices)
app/models/membership.rb            # Membership: subscriber ↔ tier link with Stripe subscription state
app/models/subscriber/billable.rb   # module Subscriber::Billable (memberships, paid_member?, stripe_customer_id)
app/models/subscriber/email_preferences.rb # module Subscriber::EmailPreferences (email_frequency enum, digest cursor, signed preferences token)
app/models/mailing_list.rb           # MailingList: subscribable list ("newsletter" to readers) — name, slug, description, frequency, active, subscribe_by_default
app/models/mailing_list_subscription.rb # Join: subscribers ↔ mailing_lists (unique per pair)
app/models/mailing_list_post.rb      # Join: posts ↔ mailing_lists (which lists a post is emailed to)
app/models/subscriber/list_subscribable.rb # module Subscriber::ListSubscribable (default-list signup, selected_mailing_list_ids form writer, subscribed_to scope)
app/models/post/notifiable.rb        # module Post::Notifiable (default lists on new posts, emails lists once on the transition to published)
app/models/post/accessible.rb       # module Post::Accessible (visibility enum, content gating)
app/models/site_setting/payment_configuration.rb # module SiteSetting::PaymentConfiguration (Stripe keys)
```

Keep model files under 200 lines — extract behavior into concerns when they grow.

### Controller Pattern
Skinny controllers that delegate to models/services. Controllers handle only HTTP concerns.

`Admin::EditorResource` (`app/controllers/concerns/admin/editor_resource.rb`) is a shared concern for the three editor-backed admin controllers (`Admin::PostsController`, `Admin::PagesController`, `Admin::NewslettersController`). It sets `layout :choose_layout` (editor layout on `new`/`edit`/`create`/`update`, `"admin"` elsewhere — declared per-controller via `uses_editor_layout "admin_editor"` etc.) and provides `respond_with_saved(record, notice:, status:)` / `respond_with_errors(record, template)` for the shared HTML+JSON `respond_to` branches on `create`/`update`. Including controllers implement `resource_json(record)` and `edit_path_for(record)` to supply their resource-specific JSON payload and redirect target; anything else that differs (e.g. `Post#create_version_if_needed!` after update) stays in the controller.

### Service Layer
- **Form Objects** for multi-model input (e.g., `Registration`)
- **Service Objects** in `app/services/` for business operations (e.g., `Ai::SystemPrompts`, `Ai::PostContextBuilder`, `Ai::ImagePromptSuggester`, `MarkdownRenderer`, `Mcp::Tools::*`)
- **Query Objects** in `app/queries/` for complex queries (e.g., `PostViewsQuery`, `SubscriberGrowthQuery`, `PostEngagementQuery`, `ReferrerAnalyticsQuery`, `SegmentSubscribersQuery`)

### Internationalization (i18n)
Site-wide locale configured via `SiteSetting.locale` (default: `"en"`). The `SiteSetting::Localization` concern defines `SUPPORTED_LOCALES` and validates the locale value. `ApplicationController` sets `I18n.locale` from the site setting on every request. All UI strings live in `config/locales/en.yml` and `config/locales/es.yml`. To add a new locale: add the language code to `SUPPORTED_LOCALES` in `app/models/site_setting/localization.rb`, add it to `config.i18n.available_locales` in `config/application.rb`, and create the corresponding YAML file in `config/locales/`.

Dates and times are rendered via `l(value, format: :short|:long|:with_time)`, never `strftime`, so they follow the site locale. Named formats live under `date.formats`/`time.formats` in each locale file — `date.formats` (`short`, `long`) for date-only values and `time.formats` (`short`, `long`, `with_time`) for values that include a time of day. Because there's no `rails-i18n` gem, each locale file also defines its own `date.month_names`/`abbr_month_names`/`day_names`/`abbr_day_names` and `time.am`/`pm` — without these, `%B`/`%A`/`%p` in a format string raise `I18n::MissingTranslationData` for any locale ActiveSupport doesn't ship a default for (i.e. anything but English). **`l` dispatches on the argument's class**: a `Date` uses `date.formats`, while a `Time`/`ActiveSupport::TimeWithZone` uses `time.formats` — so a date-only display of a timestamp column (`created_at`, `published_at`, etc.) must call `l(value.to_date, format: :short)`, not `l(value, format: :short)`, or it silently pulls from `time.formats` instead. `ApplicationHelper#published_on(record, **html_options)` wraps this for the `<time datetime="...">` tag used by post card partials (`posts/_post`, `posts/_related_posts`, `posts/show`, `admin/posts/_preview`).

### Background Jobs
Solid Queue (database-backed). Jobs organized by domain in `app/jobs/`. Recurring tasks configured in `config/recurring.yml`. **Solid Queue loads only the block named after `Rails.env` when one exists** (it does not merge top-level tasks into it), so tasks every environment needs go under the `default: &default` anchor that each environment block merges with `<<: *default`; production-only tasks sit in the `production:` block beside the merge. A task added at the top level would silently never run in production.

### SiteSetting Caching
`SiteSetting.current` is memoized on `Current.site_setting` (`app/models/current.rb`) for the lifetime of a request or job — `first_or_create!` only runs once per request instead of on every call. An `after_commit` callback on `SiteSetting` clears the memoized value so an update within the same request is not stale. `SiteHelper` exposes a `site_setting` helper method that views should call instead of `SiteSetting.current` directly. `Newsletter::Templatable#resolved_*` methods accept an optional `site` argument so `email_settings` can pass down a single fetched record rather than re-querying per field.

### Admin Settings & Masked Secrets
`Admin::SettingsController` and `Admin::NewsletterSettingsController` share their `edit`/`update` actions via the `Admin::SiteSettingsResource` concern (`app/controllers/concerns/admin/site_settings_resource.rb`); each controller only defines `site_setting_params`, `redirect_path`, and `notice_key`. The `SiteSetting::MaskedSecrets` concern (`app/models/site_setting/masked_secrets.rb`) owns the "••••••••" placeholder mask for encrypted keys (`SECRET_ATTRIBUTES`): `assign_attributes_ignoring_mask(attrs)` drops any secret attribute whose submitted value is still the mask (leaving the stored key unchanged), while a blank value still clears it; `masked_value_for(attribute)` is what views call to render the mask (or "") for a password field's current value.

### AI Integration
Uses the **RubyLLM** gem for a unified LLM interface across providers (Claude for text, Gemini/OpenAI for images). API keys are stored with Active Record Encryption on `SiteSetting` — key presence enables a feature, `nil` disables it (no separate toggle). `Ai::Client` (`app/services/ai/client.rb`) is the single place that calls `RubyLLM.configure` — `configure!(settings)` sets all three provider keys, `chat(settings)` configures and returns a `RubyLLM.chat(model: settings.ai_model_name)`, and `paint(prompt, settings)` configures and delegates to `RubyLLM.paint`. `Ai::Configurable#configure_ruby_llm!` and `AiResponseJob`/`GenerateFeaturedImageJob`/`Admin::Ai::FeaturedImagesController` all delegate to it instead of building their own `RubyLLM.configure` block. `configure_ruby_llm!` runs as a `before_action` only on `Admin::Ai::BaseController` (and subclasses) — not on `Admin::BaseController` — so non-AI admin pages don't decrypt AI API keys on every request. AI controllers are nested under `admin/posts/:id/ai/` and streaming responses use Turbo Streams + Solid Cable (`AiResponseJob` broadcasts chunks). `AiResponseJob` precomputes the latest user message id once before replaying chat history, rather than re-querying it per message in the loop.

### Custom Static Pages
Pages (`Page` model) provide custom static content at top-level URLs (`/:slug`). The catch-all route **must remain last** in `config/routes.rb` (after admin namespace and health check) to avoid intercepting other routes. Pages use `admin_page_editor` layout (simplified editor without AI/preview). Reserved slugs (admin, posts, feed, etc.) are validated at the model level. Pages are **not** added to the site menu automatically — link them from Navigation (below).

### Navigation Menus
`NavigationItem` (`label`, `url`, `location` enum `header`/`footer`/`social` with `validate: true`, `position`, `open_in_new_tab`) is the single source of the public site's menus — the header nav (`layouts/shared/_header_nav`), footer text links, and footer social icons (`layouts/shared/_footer`). There is no hardcoded "Home" link and no per-page "show in navigation" flag; the `CreateNavigationItems` migration converted the old `pages.show_in_navigation`/`pages.position` columns (since dropped) into header items. URLs must be an internal path starting with a single `/`, an `http(s)` URL, or `mailto:` (social items: `http(s)` only) — this validation is what keeps `javascript:` hrefs out of the layout, so don't loosen it. New items (and items moved to another location) are appended via `before_save`; `NavigationItem.reposition!(location, ids)` rewrites positions for drag-and-drop and `#move(:up/:down)` is the keyboard/no-JS fallback.

`NavigationItem.for_location(location)` is what views call — it caches each location's ordered records in `Rails.cache` (Solid Cache in production; `null_store` in test, so cache tests swap in a `MemoryStore`), and `after_commit` plus `reposition!` (which uses `update_columns`) expire all locations. Bump `NavigationItem::Cacheable::CACHE_VERSION` if the cached records' shape changes. `NavigationItem::SocialPlatform#social_platform` maps the URL host to a platform (unknown hosts → `:website`, `/@user` paths → `:mastodon`), and `NavigationHelper#social_icon(platform)` renders the matching Simple Icons (CC0) SVG from `NavigationHelper::SOCIAL_ICONS`; `navigation_link(item)` adds `target="_blank" rel="noopener noreferrer"` when `open_in_new_tab`.

Admin UI is `Admin::NavigationItemsController` at `/admin/navigation` (admin role only; linked from the sidebar and Settings): one page with a section per location, inline add forms, and a generic `sortable` Stimulus controller (native HTML5 drag-and-drop within one list, PATCHes `{ location, ids }` to `reorder`, reloads the page if that fails). Up/down arrows hide at list edges via Tailwind `group-first/item`/`group-last/item`, so they stay correct after a drag without a re-render. The JSON export includes `navigation_items`.

### Post Editor
Uses the `admin_editor` layout. Autosave triggers on a 3-second debounce, serializing `#post_form` FormData. The editor drawer is a tabbed panel (AI + Settings + Versions). Settings fields use `form="post_form"` attribute with event listeners on the settings tab container to trigger autosave.

### Editor Layout Composition
`admin_editor`, `admin_page_editor`, and `newsletter_editor` are thin compositions of shared partials in `app/views/layouts/editor/`: `_head` (`default_title:`, `fonts: true` — newsletter editor passes `fonts: false` to skip `font_stylesheet_tags`/`typography_style_tag`), `_top_bar` (`back_label:`), `_overlay`, and `_discard_modal` (`title:`, `message:`; the "Keep editing"/"Discard" button labels are shared under `layouts.editor.*` in the locale files rather than duplicated per layout). Flash messages across all six layouts (three editors, `admin`, `admin_auth`, and the public `application` layout) render through `layouts/shared/_flash` (`variant: :editor | :admin | :auth | :public`, default `:editor`) so `notice`/`alert` markup lives in one place; the public variant uses the `.flash`/`.flash--notice`/`.flash--alert` BEM classes in `app/assets/tailwind/components/_flash.css`. `Admin::PostsController` sets `@chat` via a `set_chat` before_action (on `edit`/`update`, when `SiteSetting.current.ai_configured?`) rather than the layout performing the `Chat.find_or_create_for` lookup/write itself.

`editor_drawer_controller.js` owns only drawer chrome — open/close, pin (persisted through `lib/storage.js`), tab switching, Escape, and Cmd/Ctrl+Shift+A. It carries no AI-specific targets or values: it derives whether an AI tab exists from the presence of a `data-tab="ai"` tab button, and `showTab` dispatches `editor-drawer:tab-shown` (with `{ tab }`) **on the panel target**, so controllers mounted inside the panel can react. `ai_chat_controller.js` owns AI chat transport (send, quick actions, clear conversation, Enter-to-send, scroll-to-bottom) and is mounted on the panel root in `admin/posts/_editor_panel.html.erb` only when `ai_available` — the page and newsletter editors render their own panels and never load it. It POSTs to `/admin/posts/:slug/ai/messages` via `requestTurboStream` (from `lib/request.js`) rather than building a hidden `<form>`, and reads title/subtitle from the autosave controller's form (`formSelectorValue`) instead of a document-wide `querySelector`.

### Post Content & Search Indexing
`Post#body_plain` is the canonical plain-text source for a post's content — `Post#excerpt(length)` truncates it and is used by `seo_description`, the featured-post teaser, and the gated-content teaser. Never re-derive plain text from `content.to_plain_text` outside of `Post::Searchable#update_body_plain`; that callback only recomputes `body_plain` when `content` actually changed (`before_save :update_body_plain, if: -> { new_record? || content.changed? }`), so a settings-only save (autosave, love counter, status toggle) skips the Nokogiri parse. `calculate_reading_time` is similarly guarded with `will_save_change_to_body_plain?`. The `posts_fts_update` SQLite trigger has a `WHEN` clause so the FTS5 row is only deleted/reinserted when `title`, `subtitle`, or `body_plain` actually changed. Run `rake posts:backfill_body_plain` to populate `body_plain` for posts created before the column existed.

### Post Versioning
`PostVersion` stores full snapshots (title, subtitle, content HTML, body plain text) on each save. Auto-versioning triggers on update with a 5-minute cooldown (`Post::Versionable`). Manual "Save version" button in the editor drawer's Versions tab. Diff view uses the `diffy` gem for plain-text comparison. "Restore" replaces the post's current content. Max 50 versions per post, pruned inline on version creation. Admin CRUD at `/admin/posts/:id/post_versions`.

### SEO Head Tags
`MetaTagsHelper#seo_head_tags(title:, description:, url:, type:, image:, extra_tags:, json_ld:)` emits a page's whole SEO `<head>` sequence — description meta, Open Graph group, `extra_tags`, Twitter card group, canonical link, optional JSON-LD — in that fixed order. Every public page view calls it once instead of chaining `meta_description_tag` / `open_graph_tags` / `twitter_card_tags` / `canonical_tag` / `json_ld_tag`; `meta_tags_for_post` is a thin wrapper that passes the post's `article:*` metadata through `extra_tags` (which is why the slot sits between the OG and Twitter groups). The image is resolved once per call and shared by both the OG and Twitter groups.

The helpers are split by responsibility: `MetaTagsHelper` (page title + meta/OG/Twitter/canonical tags), `StructuredDataHelper` (`json_ld_tag`, `json_ld_for_post`, `json_ld_for_author`, `json_ld_breadcrumb_list`), and `BreadcrumbsHelper` (`breadcrumb_items`, `breadcrumb_navigation`). `breadcrumb_items` returns `{name:, path:, url:}` entries and is the single source for both the visible breadcrumb nav (uses `:path`) and the BreadcrumbList JSON-LD (uses `:url`); the last entry has neither because it is the current page. Rails includes all helpers in views automatically, but helper *tests* must `include` each module they reach into (e.g. `StructuredDataHelperTest` includes `BreadcrumbsHelper` and `ImageOptimizationHelper`).

Image URLs are memoized for the request: `SiteHelper#default_og_image_url` uses a `defined?` guard so a nil result (no OG image configured) isn't recomputed, and `ImageOptimizationHelper#post_og_image_url(post)` caches `optimized_og_image_url` per post id so the head tags and the Article JSON-LD share one variant lookup. `Post#seo_description` and `Page#seo_description` are memoized for the same reason — a page renders them three or four times.

### BEM CSS Components
Frontend component styles use BEM (Block Element Modifier) methodology in `app/assets/tailwind/components/`. Each component gets its own file (e.g., `_post-card.css`) imported via the `_index.css` manifest. Theme variables (fonts, colors) are defined in the `@theme` block of `application.css` — Tailwind v4's `@theme` directive cannot be extracted to a separate file. `SiteHelper` methods inject runtime overrides for admin-configurable fonts and colors.

### Admin Shared Partials
Common admin view markup is factored into partials under `app/views/admin/shared/`: `_page_header` (`title:`, `action:` — an optional safe-HTML block for the header's trailing button/links), `_empty_state` (`description:`, optional `title:`/`action_label:`/`action_path:`) for the "no records yet" dashed-border box, `_form_errors` (`record:`, optional `heading:` and `wrapper_class:`) for the red error-summary box above a form, and `_stat_card` (`label:`, `value:`, optional `trend:`/`note:`) for the white bordered metric tiles on dashboard/analytics pages — `_stat_card_bordered` covers the differently-styled tiles on the revenue page. `AdminHelper` adds matching helper methods: `status_badge(label, color_class:, size: :md/:sm)` renders the pill used for post/newsletter/membership/subscriber/token statuses (with per-model `*_status_color` helpers supplying the color classes), and `admin_button_classes(variant: :primary/:outline, extra:)`, `admin_input_classes(extra:)`, `admin_th_classes` return the shared Tailwind class strings for buttons, form inputs, and table header cells so views interpolate them instead of repeating the literal class lists.

### Key Stimulus Controllers
`autosave`, `editor_drawer`, `ai_chat`, `tag_select`, `custom_select`, `streaming_markdown`, `ai_image_modal`, `typography_preview`, `markdown_preview`, `traffic_chart`, `segment_builder`, `comment_edit`, `share`, `sortable`, `bookmark`, `reading_list`

### Shared JS Modules
`app/javascript/lib/` holds framework-agnostic helpers shared across Stimulus controllers (pinned via `pin_all_from "app/javascript/lib", under: "lib"` in `config/importmap.rb`, imported as `lib/<name>`).

`lib/request.js` centralizes the CSRF-token meta lookup and the three fetch idioms used throughout the app: `request(url, opts)` (sets `X-CSRF-Token`, JSON-encodes a plain object body, form-encodes a `URLSearchParams` body, passes `FormData` through untouched), `requestJSON(url, opts)` (parses the JSON response and throws with `data.error` when the response isn't ok), and `requestTurboStream(url, opts)` (sets the Turbo Stream `Accept` header and renders the response via `Turbo.renderStreamMessage`). Controllers that POST or fetch should use these helpers instead of duplicating the CSRF meta-tag lookup.

`lib/dom.js` holds `escapeHtml(str)` (the single HTML-escaping implementation in the app — `lib/markdown.js`, `custom_select` and anything else interpolating untrusted text into a template literal import it rather than rolling their own) and `capitalize(str)` (used to build Stimulus `has<Name>Target` property names in `email_branding_preview`).

`lib/click_outside.js` exposes `useClickOutside(controller, { onClickOutside })`, which returns `{ observe, unobserve }` instead of registering immediately. Dropdown controllers call `observe()` in `open()` and `unobserve()` in `close()` and `disconnect()`, so a `document` click listener only exists while a dropdown is actually open.

`lib/listbox.js` provides `ListboxNavigator`, the shared Up/Down/Home/End/Enter/Space/Escape (plus optional type-ahead) handling for `tag_select` and `custom_select`. It owns only the highlighted index; everything visual or app-specific is injected — `getItems`, `highlight(item, on)`, `onSelect(item)`, `onEscape(event)`, `clearHighlight`, `isOpen`/`onOpenRequest`, `getItemText`, and the `typeAhead`/`homeEnd`/`selectOnSpace` flags. `handleKeydown(event)` returns whether it handled the key, so callers can fall through to their own (e.g. `tag_select`'s Backspace-removes-last-pill). **Both selects bind keydown to `this.element`, not `document`, and call `stopPropagation()` on Escape** — that is what lets the first Escape close an open dropdown and only the next one reach `editor_drawer`'s document-level handler and close the drawer. Do not move either listener back to `document`; the drawer no longer probes for open dropdowns in the DOM.

`app/views/shared/_tag_select.html.erb` keeps the pill, option, and checkmark markup in `<template>` elements (`pillTemplate`, `optionTemplate`, `checkmarkTemplate`) that `tag_select_controller` clones, so the styling lives in one place and the controller sets text via `textContent` instead of building HTML strings.

`lib/svg_chart.js` is the shared renderer for all hand-rolled SVG charts (no external charting library) — `svgEl(name, attrs, text)` builds namespaced SVG elements, `renderBarChart(container, entries, opts)` draws gridlines/bars/x-labels for `growth_chart` and `traffic_chart` (which differ only in `minBarWidth`, `labelInterval`, and label formatter — `formatMonthLabel`/`formatDayLabel`), and `renderSparkline(container, values, opts)` draws the dashboard's line+area sparkline (`chart_controller`). All three controllers render into a persistent container element via `container.replaceChildren(...)`, so redrawing (e.g. on data change) doesn't destroy the target. The growth chart's Monthly/Cumulative toggle uses Stimulus `static classes` (`data-growth-chart-active-class` / `-inactive-class`) with `classList.add/remove` rather than string replacement on `className`.

`lib/timers.js` keeps Stimulus timers tied to the controller lifecycle — an untracked `setTimeout` survives `disconnect()` and keeps firing against a detached controller after a Turbo navigation. `debounce(fn, ms)` returns a wrapped function with a `.cancel()` method (used by `handle_check`); `useTimeouts()` returns `{ set, clear, clearAll }` that tracks every timeout id so `disconnect()` can `clearAll()` (used by `autosave` and `custom_select`). `autosave` also keeps a `connected` flag so a response landing after disconnect can't queue a new retry, which is what made a failed save loop forever after navigating away.

`lib/storage.js` exports a `storage` object (`get`/`set`/`remove`/`getBoolean`) wrapping `localStorage` in try/catch — the API throws outright in private-browsing/blocked-site-data configurations, and an unguarded read inside `connect()` aborts the whole controller. `editor_drawer` and `theme_toggle` go through it; the inline theme boot script in `layouts/application.html.erb` has its own try/catch since it runs before the module graph loads. The `theme` key name is shared between that script and `theme_toggle`, so don't rename it.

### Author Profiles
Profile data (bio, avatar, social links) lives on the `Identity` model via `Identity::Profileable` concern. Public author pages at `/authors` (index) and `/authors/:handle` (show) are served by `AuthorsController`. Admin profile editing at `/admin/profile` via `Admin::ProfilesController`. Author names on posts link to their profile pages. Bios support markdown via `MarkdownRenderer`.

### Traffic Analytics
`Admin::TrafficController` provides a dedicated deep-dive traffic page at `/admin/traffic` with referrer domain breakdown, UTM campaign/source/medium tables, and a configurable range selector (7d/30d/90d/all). `PostView` stores `referrer_domain`, `utm_source`, `utm_medium`, and `utm_campaign` extracted at write time by `TrackPostViewJob`. `ReferrerParser.call(referrer)` (`app/services/referrer_parser.rb`) is the single source of referrer-parsing logic (domain, source classification, UTM extraction, truncation, `URI::InvalidURIError` handling) — both `TrackPostViewJob` and the `referrer:backfill` rake task use it; the rake task slices out `domain`/`utm_*` (renaming `domain` to `referrer_domain`) so it doesn't overwrite `source` on existing rows. `ReferrerAnalyticsQuery` provides aggregation methods. Use `rake referrer:backfill` to populate columns from existing referrer URLs.

### Analytics Range Parsing
`AnalyticsRange` (`app/models/analytics_range.rb`) is a value object that resolves a `range` param (e.g. `"7d"`, `"30d"`, `"6mo"`, `"all"`) to a `since` timestamp via `AnalyticsRange::PRESETS`. `AnalyticsRange.parse(param, default:, allowed:)` falls back to `default` when the param isn't in `allowed`. The `Admin::AnalyticsRangeable` concern (`app/controllers/concerns/admin/analytics_rangeable.rb`) adds a memoized `analytics_range(default:, allowed:)` controller helper, used by `Admin::TrafficController`, `Admin::RevenueController` (both `%w[7d 30d 90d all]`, default `"30d"`), and `Admin::GrowthController` (`%w[6mo 12mo 24mo all]`, default `"12mo"`). `Admin::DashboardController` doesn't take a `range` param but uses `AnalyticsRange.new("30d").since` to avoid repeating `30.days.ago`. The `admin/shared/range_selector` partial (`ranges:`, `current:`, `path:` locals) renders the joined-pill range-selector UI shared by the traffic and growth show views.

### Subscriber Segmentation
`SubscriberLabel` provides tagging for subscribers (name + hex color). `SubscriberLabeling` is the many-to-many join. `Segment` stores saved filter criteria as JSON (`filter_criteria` column) with a `Segment::Resolvable` concern that delegates to `SegmentSubscribersQuery`. Filters support: label matching (any_of/all_of/none_of), date ranges on `confirmed_at`, and engagement level (active/inactive derived from `newsletter_deliveries`). Newsletters have an optional `mailing_list_id` and `segment_id` — `Newsletter::Sendable#target_subscribers` starts from the list's `recipients` (or all confirmed subscribers) and, when a segment is set, narrows that audience with `segment.resolve(audience)` (`SegmentSubscribersQuery.new(criteria, scope:)` filters the given scope instead of always starting from `Subscriber.confirmed`). Admin CRUD at `/admin/subscriber_labels`, `/admin/segments`. Labels are managed per-subscriber at `/admin/subscribers/:id`.

`Segment::CriteriaBuilder` owns the flat-form ↔ `filter_criteria` JSON translation so `Admin::SegmentsController` only permits params. It declares virtual `attribute`s (`label_ids`, `label_mode`, `subscribed_after`, `subscribed_before`, `engagement`) and a `before_validation :build_filter_criteria` that assembles the `filter_criteria` hash from them (`label_ids.reject(&:blank?).map(&:to_i)`, `label_mode.presence || "any_of"`). The public readers fall back to the stored `filter_criteria` hash when the virtual attribute itself is unset, so `segments/_form` can read `segment.label_ids` / `segment.label_mode` / etc. directly instead of parsing `filter_criteria` — this is what makes the edit form round-trip from the stored JSON.

### Email & Payment Provider Adapters
`EmailService` and `PaymentService` each follow the same provider pattern: a module-level `.provider` factory builds the configured adapter from `SiteSetting` (injecting keys/secrets into the constructor rather than having the adapter read `SiteSetting` itself), and `.configured?` reports whether the feature is usable. Both adapter interfaces are declared on a `Base` class (`EmailService::Base`, `PaymentService::Base`) that the concrete adapters subclass. `EmailService::Base#deliver_newsletter(newsletter, subscriber)` is the single entry point callers use — `SendNewsletterJob` always calls `provider.deliver_newsletter`, with no provider branching in the job. `EmailService::Smtp#deliver_newsletter` enqueues `NewsletterMailer#campaign`; `EmailService::Sendgrid#deliver_newsletter` renders that same mailer and posts it via the SendGrid API (`#send_email`). SendGrid webhook event processing (updating `NewsletterDelivery` open/click/bounce/unsubscribe state) lives in `EmailService::Sendgrid::WebhookProcessor`, used directly by `Webhooks::SendgridController` — it needs no API key, since it never calls the SendGrid API. `PaymentService::Stripe` is constructed with `secret_key:`, `publishable_key:`, and `webhook_secret:` all injected by `PaymentService.provider` from `SiteSetting`, so `construct_webhook_event` never reads `SiteSetting` directly. `EmailService.from_address` is the single `SMTP_FROM` env lookup, used by both `ApplicationMailer`'s default `from` and `EmailService::Sendgrid`.

### Mailer Branding & Layout
`SiteSetting::EmailBranding#email_branding` resolves the seven branding fields (accent/background/body/heading colors with `DEFAULT_*` constant fallbacks, font stack, footer text, site name, logo URL) into one hash. `ApplicationMailer#load_email_branding` sets `@email_settings` from it (plus per-field ivars the individual mailer templates read); `Newsletter::Templatable#email_settings` merges the same hash with newsletter-specific overrides (resolved template/accent color/preheader text, social links) rather than re-deriving the color fallbacks. `ApplicationMailer#set_list_unsubscribe_headers(url)` is shared by `NewsletterMailer#campaign` and `PostNotificationMailer#new_post`, the only two mailers that set `List-Unsubscribe`/`List-Unsubscribe-Post`. `app/views/mailer/_cta_button.html.erb` (`url:, label:, color:`) is the shared CTA table used by `post_notification_mailer/new_post`, `subscriber_mailer/confirmation`, `subscriber_mailer/magic_link`, and `comment_mailer/reply_notification`. `app/views/mailer/_footer.html.erb` (`settings:`, reads `@unsubscribe_url` directly) is shared by the three newsletter templates (`newsletter_mailer/_template_*`) and by `layouts/mailer.html.erb` (used by `post_notification_mailer`, `subscriber_mailer`, `comment_mailer`) — the layout keeps its own header markup since it supports a logo fallback the newsletter's minimal template doesn't.

### Email Frequency & Digests
`Subscriber::EmailPreferences` adds `email_frequency` (enum `immediate`/`weekly`/`monthly`/`none`, `prefix: :email`, `validate: true`, default `immediate`). The frequency governs **post emails only**: `SendPostNotificationsJob` sends to `post.notification_recipients.email_immediate`, and `SendDigestsJob` covers `weekly`/`monthly`. Newsletters ignore it (`Newsletter::Sendable#target_subscribers` is unchanged), so `none` means "no post emails", not unsubscribed.

`SendDigestsJob.perform(frequency)` runs from `config/recurring.yml` (`"0 8 * * 1"` weekly, `"0 8 1 * *"` monthly, server time zone). `subscribers.last_digest_at` is a **delivery cursor**, meaning "posts published before this have reached the subscriber", not strictly "when a digest was last sent". `#digest_window_start(now)` is `last_digest_at || now - period`. The job lists `Post.live.where(published_at: since...now)` (half-open, so consecutive windows neither overlap nor gap) limited to posts on the subscriber's active mailing lists (subscribers on no active list are skipped and keep their cursor), caches the lookup per distinct (window start, list ids) pair, enqueues `DigestMailer.digest(subscriber, post_ids.first(POST_LIMIT), total_count)`, then `update_all`s `last_digest_at` to the run's `now` for that batch. A retried run therefore finds empty windows, and a subscriber with no new posts keeps their cursor. A `before_save` moves the cursor when the frequency changes: from `immediate` it jumps to now (those posts were already emailed individually), from `none` it clears (falls back to the last period), and between `weekly` and `monthly` it is kept. `DigestMailer` reloads `Post.live` at delivery time and returns without mailing if the subscriber has since unsubscribed or every post was unpublished. It shows `seo_description` as the excerpt (already public in meta tags, so it doesn't leak gated content beyond what the page head does), and uses a non-WebP featured-image variant because email clients can't render WebP.

Subscribers manage the setting at `/email-preferences` (`EmailPreferencesController`; `email-preferences` is a reserved `Page` slug). Access is either a signed `token` param (`Subscriber#email_preferences_token` / `.find_by_email_preferences_token`, message verifier purpose `"email_preferences"`, 30-day expiry, separate from the `"unsubscribe"` token) or a signed-in `current_subscriber`. `PostNotificationMailer`, `DigestMailer` and `NewsletterMailer` set `@email_preferences_url`, which `mailer/_footer` renders beside the unsubscribe link, and the unsubscribe confirmation page links to it as an alternative. In mailer templates build post links with `post_url(slug: post.slug)`: `post_url(post, slug: ...)` puts the record in the `:format` slot and yields `/posts/<slug>.<slug>`.

### Mailing Lists (Multiple Newsletters)
`MailingList` is the subscribable list readers see as a "newsletter" — **not** `Newsletter`, which is a one-off email campaign (the name was taken by #22). Lists have `name`, `slug` (`Sluggable`), `description`, `frequency` (free-text label such as "Weekly", shown to subscribers; it doesn't schedule anything), `active` and `subscribe_by_default`. Inactive lists are hidden from subscribers and the post editor and receive no post emails; subscriptions to them are kept so reactivating restores them. Admin CRUD at `/admin/mailing_lists` (`Admin::MailingListsController`, open to writers like labels/segments), with confirmed-subscriber counts from `MailingList.subscriber_counts`.

Subscribers: `Subscriber::ListSubscribable` joins every `MailingList.subscribed_by_default` list `after_create`. The `/email-preferences` page shows a checkbox per active list (only when more than one is active) that writes `selected_mailing_list_ids`, a virtual attribute synced `after_save` — it only replaces subscriptions to *active* lists. Global unsubscribe (`unsubscribed_at`) is separate and always wins, since recipients are always built from `Subscriber.confirmed`.

Posts: `Post::Notifiable` gives new posts the default lists in `after_initialize` **unless** `mailing_list_ids=`/`mailing_lists=` was called (the editor always submits a hidden `""` plus its checkboxes, so unchecking every list means "email nobody"). The new-post email fires from an `after_commit` on any transition to `published` — not from `publish!` — so the admin editor (which saves `status` directly), the API, MCP and `PublishScheduledPostsJob` all notify. `posts.subscribers_notified_at` is claimed with a conditional `update_all` before `SendPostNotificationsJob` is enqueued, so unpublish/republish never emails twice; code that creates already-published posts without wanting emails (seeds, imports) should set `subscribers_notified_at`. `Post#notification_recipients` is `Subscriber.confirmed.subscribed_to(mailing_lists.active)`. API/MCP don't accept list ids yet, so posts created there use the default lists.

The `CreateMailingLists` migration folds existing installs into one default "Newsletter" list (every non-unsubscribed subscriber, every post) and stamps published posts as notified. The JSON export includes `mailing_lists` and `mailing_list_ids` on posts and subscribers.

### Paid Memberships (Stripe)
Prose supports Ghost-style paid memberships with Stripe integration. Feature is gated — UI only appears when Stripe keys are configured in `SiteSetting`. Direct Stripe API keys (not Stripe Connect), stored encrypted like AI keys. 3-level post visibility: `public` (default), `members_only` (free sign-in), `paid_only` (subscription required). `PaymentService::Stripe` handles checkout sessions, billing portal, subscriptions, and webhook event construction — see Email & Payment Provider Adapters above. `StripeWebhookJob` processes checkout completions, subscription updates/deletions, and payment failures (idempotent) — it's a thin dispatcher that delegates status mapping and attribute syncing to `Membership::StripeSyncable`. `SyncStripeSubscriptionJob` runs daily to keep membership status in sync, also via `StripeSyncable`. Admin controllers at `/admin/payment_settings`, `/admin/membership_tiers`, `/admin/memberships`, `/admin/revenue`. Public pricing page at `/memberships`. Content gating in `posts/show` — `PostsController` sets `@can_view` via `MembershipAccess` concern. Query objects: `RevenueQuery` (MRR, ARR, churn) and `MembershipGrowthQuery` (new/canceled/net by month).

`PaymentGating` (`app/controllers/concerns/payment_gating.rb`), included on `ApplicationController`, is the single source of Stripe-configuration gating: `helper_method :payments_configured?` (memoized per request via a `defined?(@payments_configured)` guard, mirroring `SiteHelper#default_og_image_url`) replaces direct `PaymentService.configured?` calls in views (`posts/_paywall`, `layouts/_admin_sidebar_content`, `admin/membership_tiers/_tier_row`/`_tier_card`), and `require_payments_configured(redirect_to: root_path, alert: nil)` is a `before_action` that redirects when Stripe isn't configured — `MembershipsController` uses the default (silent redirect to `root_path`), while `Admin::RevenueController` overrides it to call `super(redirect_to: admin_root_path, alert: t("flash.payments.not_configured"))`. `Admin::MembershipTiersController` and `Admin::MembershipsController` are deliberately left ungated — tiers must be manageable before their first Stripe sync, and comp memberships (`Membership.grant_complimentary!`) don't require Stripe at all.

`Membership::StripeSyncable` centralizes the Stripe→Membership mapping shared by both jobs: `Membership.status_from_stripe(stripe_status)` maps a Stripe subscription status string to the `status` enum (unknown statuses fall back to `:incomplete`); `#sync_from_stripe!(subscription)` accepts either a `Stripe::Subscription` object or a string-keyed Hash (webhook payload) and updates `status`/`current_period_start`/`current_period_end`/`canceled_at` — missing period fields leave the stored value untouched, while `canceled_at` always reflects the given value (including clearing it back to `nil`) since Stripe always sends that field on a full subscription object; `Membership.activate_from_checkout!(session_data, subscription)` handles `checkout.session.completed` — looks up the subscriber by email and the tier by Stripe price ID, then `find_or_create_by!`s the membership (idempotent against duplicate webhook delivery) before calling `sync_from_stripe!`.

`Membership.grant_complimentary!(subscriber:, membership_tier:)` creates a Stripe-free active membership (100-year period) for admin-granted comps. It's invoked from the subscriber show page via `POST /admin/subscribers/:subscriber_id/memberships/comp` (`admin_subscriber_comp_membership_path`, routed to `Admin::MembershipsController#comp`) rather than a member route on `admin/memberships`, since granting a comp creates a new membership and doesn't operate on an existing one.

`MembershipTier::Syncable` runs Stripe product/price creation `after_commit` (not `after_save`) via `SyncMembershipTierJob`, so the HTTP round-trip to Stripe never holds SQLite's single writer lock open during the save transaction. The admin tiers index (`app/views/admin/membership_tiers/index.html.erb`) shows a "Syncing…" badge on any tier missing a `stripe_price_id` and subscribes to a `"membership_tiers"` Turbo Stream; `SyncMembershipTierJob` broadcasts a `broadcast_replace_to` for both the tier's mobile card and desktop row partials once the sync completes, so the badge clears without a page reload.

### Comment Features
Comments belong to `Identity` (not `Subscriber`), allowing both admins and subscribers to comment. One-level threading only. Features:
- **Edit/Delete**: Authors can edit within 15 minutes (`Comment::Editable`). Soft delete sets `deleted_at` and replaces body with "[deleted]". Admin hard-delete unchanged.
- **Reply Notifications**: Opt-in via `notify_on_reply` checkbox. `Comment::Notifiable` fires `CommentReplyNotificationJob` on reply creation. `CommentMailer#reply_notification` sends email with token-based unsubscribe (`CommentNotificationsController`).

### Reading List (Bookmarks)
Readers save posts for later with the bookmark icon rendered by `ReadingListHelper#bookmark_button` in every post card's meta row (`posts/_post`) and on `posts/show`, and view them at `/reading-list` (`ReadingListController#show`, also linked from the masthead icon group `.masthead__actions`; `reading-list` is a reserved `Page` slug). `ReadingListItem` belongs to `Identity` (like `Love`/`Comment`), so staff users signed into the public site get a list too.

**The button markup is reader-agnostic** — it lives inside the fragment-cached `posts/_post` partial, so it renders `hidden` with `aria-pressed="false"` and `bookmark_controller.js` reveals it and applies the saved state client-side. Never render per-reader state into it. `lib/reading_list.js` is the single source of truth for every button: anonymous readers' ids live in localStorage (`reading_list` key, JSON array, newest first); signed-in readers' ids come from `<meta name="reading-list" content="account" data-post-ids=... data-items-url=... data-import-url=...>` emitted by `ReadingListHelper#reading_list_meta_tag` in the public layout (anonymous: `content="local"`). Account state is held in module memory and only re-seeded from the meta when its `data-key` changes (sign-in/out), because a Turbo restoration visit restores a head snapshot that may predate the reader's last toggle; toggles also write back to the meta so later snapshots are current. Every change dispatches `reading-list:changed` on `window`, which each button listens for (plus `storage` for other tabs).

Signed-in toggles are optimistic and go through the JSON endpoints `POST /reading-list/items` / `DELETE /reading-list/items/:post_id` (`ReadingListItemsController`); the first page a reader loads signed in POSTs any localStorage ids to `/reading-list/import` (`ReadingListImportsController` → `Identity#import_reading_list!`, which skips non-live/duplicate ids and respects the cap) and then clears localStorage. All three return `{ post_ids: [...] }` and share `ReadingListJson` (401 JSON instead of a redirect when signed out). Anonymous readers' `/reading-list` page renders a turbo frame whose `src` `reading_list_controller.js` points at `GET /reading-list/posts?ids=...` (`ReadingListPostsController`, live posts only, in the given order) plus a subscribe prompt to sync across devices. Only live posts are ever listed; saved drafts/unpublished posts stay in the table but are filtered out by `Post.live`.

### Social Embeds
`XPost` and `YouTubeVideo` models with oEmbed fetching, embedded in rich text via ActionText. Both include `OembedAttachable` (`app/models/concerns/oembed_attachable.rb`), which includes `ActionText::Attachable`, derives `to_attachable_partial_path`/`to_trix_content_attachment_partial_path` from `model_name` (e.g. `"x_posts/x_post"`), and implements `find_or_create_from_url(url)`: it normalizes the URL, bails out via the overridable `valid_oembed_url?` class method (used by `YouTubeVideo` to reject URLs with no extractable video ID), then `find_or_create_by(url:)` — calling the private `assign_oembed_url_attributes` hook (overridden by `YouTubeVideo` to set `video_id` before validation) and fetching+applying oEmbed data via the per-model `oembed_endpoint`/`apply_oembed(data)` methods. `OembedFetcher.fetch(endpoint_url)` (`app/services/oembed_fetcher.rb`) is the single place that performs the blocking `Net::HTTP` oEmbed request (including the `cert_store.flags = 0` reset for system OpenSSL builds); it returns `{}` and logs a warning on any error or non-JSON response instead of raising. `paste_embed_controller.js` is the single Stimulus controller mounted on the post editor's writing area — it holds a module-level `PROVIDERS` table (regex pattern per provider) and a `data-paste-embed-urls-value` JSON object (`{ xPost:, youtube: }`, built from the admin routes in `_form.html.erb`) mapping each provider to its create endpoint; on paste it tests the clipboard text against each pattern, posts to the matching URL via `requestJSON`, and inserts the returned attachment via `lib/action_text.js#insertAttachment(editor, { sgid, html })`. `lib/twitter_widgets.js#loadTwitterWidgets()` memoizes the `widgets.js` script tag/load so `x_post_widget_controller.js` — mounted once per tweet by `x_posts/_x_post.html.erb` — never appends a duplicate script tag for a post with multiple embedded tweets; the redundant wrapper-level mount on `posts/show`/`admin/posts/_preview` was removed since the per-tweet mount is sufficient.

### MCP Server (Model Context Protocol)
Prose exposes an MCP endpoint at `POST /mcp` for AI assistants to manage blog content. See `docs/mcp_setup.md` for the full client setup guide.

**Authentication**: Bearer token via `Authorization` header. Tokens are prefixed with `prose_`, stored as SHA256 digests (never raw), and support instant revocation. The `ApiToken` model handles generation, lookup, and usage tracking (last used time/IP). The `User::ApiTokenable` concern adds `has_many :api_tokens` and a convenience `generate_api_token!` method.

**Controller**: `Mcp::SessionsController` (inherits `ActionController::API`) — one `handle` action routed for `GET`/`POST`/`DELETE /mcp` that authenticates the token, sets `Current.user`, and hands the request to the gem's `MCP::Server::Transports::StreamableHTTPTransport` (built per request with `stateless: true, enable_json_response: true`, returning a Rack triple the action renders). Always go through the transport, never `MCP::Server#handle_json` directly. The transport handles the Streamable HTTP rules that real clients depend on: `202` for notifications, `405` for `GET` (no SSE stream), Accept/Content-Type validation, the `MCP-Protocol-Version` header, and the 2026-07-28 per-request envelope. Claude Desktop reaches it through the `mcp-remote` stdio bridge, since `claude_desktop_config.json` can't send a bearer header to a remote URL. Clients (and tests) must send `Content-Type: application/json` plus `Accept: application/json, text/event-stream`. `wrap_parameters false` keeps Rails from parsing the body so the transport returns its own JSON-RPC parse error, and `MAX_REQUEST_BYTES` (16 MB) raises the transport's 4 MiB default for base64 uploads. DNS-rebinding checks are disabled in favor of Rails `config.hosts`. Rate limited at 60 req/min per IP.

**Tool architecture**: 14 tools in `app/services/mcp/tools/`, all inheriting from `Mcp::Tools::Base` (itself an `MCP::Tool` subclass). Each declares a `description`, `input_schema`, and `call(server_context:, **params)` class method. Tools are registered via `Mcp::ToolRegistry.all`, which excludes `Base`. `Base` provides private class-level helpers shared across tools: `with_post` (wraps `find_post` with a `"Post not found: ..."` error envelope on `ActiveRecord::RecordNotFound`) and `success`/`failure` response envelope builders. The lookups themselves — `find_post` (slug-or-numeric-ID), `find_category`, `find_tag`, `find_or_create_tags`, `decode_upload` (base64 → `[StringIO, content_type]`) — live in `Mcp::ContentLookup` (`app/services/mcp/content_lookup.rb`), included into `Base`'s singleton class and into `Api::V1::BaseController`, so MCP and REST share one implementation. `MCP::Tool.inherited` resets description/schema per subclass and `tool_name` derives from the leaf class name, so the intermediate `Base` class doesn't affect tool names or schemas.

```
app/services/mcp/
├── tool_registry.rb          # Central registry of all tool classes
├── post_serializer.rb        # Consistent post JSON serialization
├── markdown_converter.rb     # Markdown → HTML (Commonmarker, GFM)
└── tools/
    ├── base.rb                # Mcp::Tools::Base — shared post/category/tag/upload helpers, response envelopes
    ├── list_posts.rb          # Filter by status/category/tag/search, paginated
    ├── get_post.rb            # Full post by slug or ID
    ├── create_post.rb         # New draft from markdown
    ├── update_post.rb         # Partial updates
    ├── delete_post.rb         # Permanent deletion
    ├── publish_post.rb        # Immediate publish + subscriber notifications
    ├── schedule_post.rb       # Future publication (ISO 8601)
    ├── unpublish_post.rb      # Revert to draft
    ├── get_site_info.rb       # Site metadata + counts
    ├── list_categories.rb     # Categories with post counts
    ├── list_tags.rb           # Tags with post counts
    ├── create_tag.rb          # Find or create tag
    ├── upload_asset.rb        # Base64 file → ActiveStorage blob
    └── set_featured_image.rb  # Attach featured image to post
```

**Admin UI**: `Admin::ApiTokensController` with token CRUD at `/admin/api_tokens`. Admins see all tokens; writers see only their own. Raw token shown once via flash on creation.

### REST API
A versioned JSON REST API lives alongside MCP at `/api/v1/` for non-MCP integrations (mobile apps, custom frontends, data pipelines). See `docs/api_setup.md` for the full endpoint reference.

`Api::TokenAuthenticatable` (`app/controllers/concerns/api/token_authenticatable.rb`) is the single bearer-token auth implementation, shared by `Mcp::SessionsController` and `Api::V1::BaseController` — it extracts the token, looks it up via `ApiToken.find_by_raw_token`, sets `Current.user`, and calls `render_unauthorized(message)` on failure; each including controller overrides `render_unauthorized` for its own error envelope shape (JSON-RPC for MCP, `{ error: }` for REST). `Api::V1::BaseController < ActionController::API` also rate limits at 60 req/min (matching MCP — `rate_limit` is declared **before** including `Api::TokenAuthenticatable` in both controllers so unauthenticated requests are throttled too; keep that order), maps `ActiveRecord::RecordNotFound`/`RecordInvalid`/`ActionController::ParameterMissing` to `404`/`422`/`400` JSON errors, and provides a `paginate(scope)` helper (`page`/`per_page`, max 50) that sets `X-Total-Count` and a GitHub-style `Link` header.

Resource controllers (`Api::V1::PostsController`, `CategoriesController`, `TagsController`, `SiteController`, `AssetsController`) reuse existing model logic directly (`Post.for_listing`/`.search`, `Publishable#publish!`/`schedule!`/`revert_to_draft!`, `Sluggable`) and `Mcp::PostSerializer`/`Mcp::MarkdownConverter` for consistent JSON output between MCP and REST — there is no separate REST-specific post serializer.

### Webhooks (Outbound)
`Webhook` (url, `events` json array, `signing_secret` encrypted, `active`, `consecutive_failures`, `last_triggered_at`, `last_response_code`) lets third-party services subscribe to site events. `Webhook::EVENTS` is the fixed list: `post.published`/`updated`/`deleted`/`scheduled`/`unpublished`, `subscriber.created`/`deleted`, `comment.created`/`approved`. `WebhookDispatcher.deliver(event, data)` finds active webhooks subscribed to `event` and enqueues `DeliverWebhookJob` per webhook; `Webhooks::Sender.post` does the raw `Net::HTTP` POST (5s open / 10s read / 10s write timeouts, redirects are never followed), raising `Webhooks::Sender::DeliveryError` on a non-2xx response or network failure. Requests carry `X-Prose-Timestamp` (unix seconds), `X-Prose-Event`, `X-Prose-Delivery` (the job id, stable across retries), and `X-Prose-Signature: sha256=<hex>` where the hex is `HMAC-SHA256(signing_secret, "#{timestamp}.#{raw_body}")` — receivers should compare in constant time and reject stale timestamps. **SSRF protection** lives in `Webhooks::UrlGuard`: `Webhook` validation rejects `localhost`/`*.local`/`*.internal` hosts and literal loopback/private/link-local/reserved IPs (incl. IPv4-mapped IPv6), and `Sender` calls `UrlGuard.resolve!` at send time — every DNS answer must be public, and the connection is pinned to the vetted IP via `Net::HTTP#ipaddr=` so a DNS rebind can't redirect it. The job's `retry_on` (polynomial backoff, `DeliverWebhookJob::MAX_ATTEMPTS` = 3) retries failed deliveries and swallows the error once exhausted; every attempt is logged as a `WebhookDelivery` row (pruned inline to the newest `Webhook::DELIVERY_LOG_LIMIT` = 100 per webhook via `Webhook#prune_deliveries!`) and rolls into `Webhook#record_delivery_result!`, which resets `consecutive_failures` on success, but only increments it on the **final** failed attempt (`count_failure:`), so one event exhausting its retries counts once; the webhook auto-disables (`active: false`) once `consecutive_failures` reaches `Webhook::MAX_CONSECUTIVE_FAILURES` (5).

Event triggers live at the **model** layer (not controllers), so both the Admin UI and MCP tools fire the same events: `Post::Webhookable` (`after_update_commit`/`after_destroy_commit`) maps a `status` transition to `post.published`/`post.scheduled`/`post.unpublished`, and fires `post.updated` (other saved changes) and `post.deleted` (destroy) **only for published posts** so draft titles/slugs never leave the site. `Subscriber.subscribe_or_sign_in!` fires `subscriber.created` only for a genuinely new subscriber (not a returning magic-link sign-in); `Subscriber#unsubscribe!` fires `subscriber.deleted` (there is no hard subscriber delete in the app, so "deleted" is modeled as unsubscribing). `Comment` fires `comment.created` on `after_create_commit` and exposes `#approve!` (used by `Admin::CommentsController#update` instead of a bare `update!(approved: true)`) which fires `comment.approved`. Per-resource payload serializers live in `app/services/webhooks/` (`PostSerializer`, `SubscriberSerializer`, `CommentSerializer`) — plain hashes with iso8601 timestamps, separate from the MCP serializers since webhook payload shape needs differ.

**Admin UI** (admin role only — both controllers `before_action :require_admin`, and the sidebar link is hidden from writers, since webhooks can exfiltrate subscriber emails): `Admin::WebhooksController` (full CRUD at `/admin/webhooks`) shows the signing secret once via flash on creation or regeneration (`POST .../regenerate_secret`), and a "Send Test Event" button (`POST .../test`) enqueues a `ping` event bypassing subscription filtering. `Admin::WebhookDeliveriesController` (`/admin/webhooks/:webhook_id/webhook_deliveries`) shows the delivery log (event, response code, timestamp, success/fail) for one webhook.

### Fediverse (ActivityPub)
Opt-in via `SiteSetting#activitypub_enabled` (Admin → Settings → Fediverse); while off, every ActivityPub route 404s (`ActivityPub::BaseController#require_federation`) and post URLs serve only HTML. The whole site is **one actor** (`@<activitypub_username>@APP_HOST`, a `Person` at `/activitypub/actor`) — there are no per-author actors. Enabling federation the first time generates an RSA-2048 keypair on `SiteSetting` (`activitypub_private_key` is encrypted); it is never rotated automatically because remote servers cache it by key id. Because the actor id is the URL `/activitypub/actor` (not the username), changing the username doesn't break existing follows, but changing `APP_HOST` does.

Everything lives under the `ActivityPub::` namespace (`app/controllers/activity_pub/`, `app/services/activity_pub/` — the directory is `activity_pub` so Rails' default inflection yields `ActivityPub`). **`ActivityPub::Urls` is the single source of every ActivityPub id**: it builds absolute URLs from `config.action_mailer.default_url_options` (APP_HOST in production), never from the request host, so a URL served by a controller matches the one a background job delivers. It also has `post_for(uri)`, which maps an incoming object id back to a live `Post` (only this host, only `/posts/:slug`). Note `post_url(slug: post.slug, ...)` — passing the record positionally *and* `slug:` puts the record in the `:format` slot (see "Slug Route Helpers" below).

Endpoints: `/.well-known/webfinger`, `/activitypub/actor`, `/activitypub/inbox` (also advertised as the shared inbox), `/activitypub/outbox` (20 newest live posts), `/activitypub/followers` and `/following` (counts only — follower identities are never published). `GET /posts/:slug` with an `application/activity+json` or `ld+json` Accept header is routed to `ActivityPub::ObjectsController` by a lambda route constraint (`ActivityPub::Negotiation`) placed before `resources :posts`, so `PostsController` stays HTML-only; `posts/show` adds a `<link rel="alternate" type="application/activity+json">`.

**Signatures**: `ActivityPub::Signature` implements draft-cavage HTTP Signatures (RSA-SHA256 over `(request-target) host date [digest]`, with a SHA-256 `Digest` of the body); the verifier requires those headers to be signed, a Date within 12h (1h future skew), and a matching digest. `ActivityPub::RequestVerifier` resolves the signer via `ActivityPub::ActorFetcher.for_key_id` (cached as a `FediverseActor`, refetched after a day or once on verification failure to pick up key rotation), and the inbox rejects any activity whose `actor` isn't the signer. `ActorFetcher` only accepts a document whose `id` equals the URI requested and whose `publicKey.owner` is that actor. **All outbound HTTP (actor fetches and deliveries) goes through `ActivityPub::HttpClient`**, which reuses `Webhooks::UrlGuard.resolve!` + `Net::HTTP#ipaddr=` pinning for SSRF protection, signs GETs too (for "authorized fetch" servers), refuses plain http in production, never follows redirects, and caps response bodies at 1 MB.

**Inbox** (`ActivityPub::InboxProcessor`): `Follow` of our actor → `FediverseActor#follow!` + an `Accept` delivered via `DeliverActivityJob`; `Undo` of a Follow/Like; `Like` of a live post → `FediverseLike` (unique per actor/post; count shown on the admin post dashboard); `Create(Note)` that is public, attributed to the signer, hosted on the signer's domain, and `inReplyTo` a live post (or a top-level fediverse comment) → a `Comment` with `approved: false` and `activitypub_uri` (unique, so redelivery is a no-op), authored by an `Identity` created lazily via `FediverseActor#ensure_identity!` (no handle). Note HTML is converted to plain text by `ActivityPub::CommentBody` — remote HTML is never stored — and a leading mention of our actor is stripped. `Delete` soft-deletes the signer's own comment, or, when the object is the actor itself, removes its follow and likes. An unverifiable `Delete` of an actor by itself (the account is gone, so its key 410s) is acknowledged with 202 and ignored. Replies to replies are dropped (comments are one level deep), and `Update(Note)` is not handled.

**Outbound** (`Post::Federatable`, mirroring `Post::Webhookable`): transition to `published` → `FederatePostJob("create")`; transition away from published or destroy → `"delete"` (a Tombstone; the URL is passed in since the post may be gone); a title/subtitle/body/visibility change on a published post → `"update"`, **enqueued with a 2-minute delay and the post's `updated_at`** so autosave bursts coalesce: the job skips itself if the post has changed since. `FederatePostJob` builds the activity (`ActivityPub::Activities` + `ActivityPub::ArticleSerializer`) and enqueues one `DeliverActivityJob` per `FediverseActor.follower_inboxes` (shared inboxes deduplicated). Articles carry full HTML only for `visibility_public?` posts — members-only/paid posts send `seo_description` + a "Read more" link — and rich-text attachments are flattened to plain `<img>`/`<a>` tags with absolute URLs, the same way the Markdown exporter does it. `DeliverActivityJob` retries (polynomial, 5 attempts) on 5xx/429/network errors, drops 4xx, and on `410 Gone` unfollows every follower behind that inbox.

Tests share `ActivityPubTestHelper` (bottom of `test/test_helper.rb`): `enable_federation!`, `remote_actor`, `signed_inbox_headers(body)` (signs as a remote actor with a fixed test key), and `with_singleton_stub` (Minitest 6 dropped `Object#stub`).

### Slug Route Helpers
`posts`, `categories`, and `tags` are routed with `param: :slug`, so call their helpers with the keyword only: `post_path(slug: post.slug)`, `category_url(slug: category.slug)`, `tag_url(slug: tag.slug)` — or pass just the record (`post_path(post)`), since `Post#to_param` returns the slug. **Never pass both** (`post_url(post, slug: post.slug)`): the explicit `slug:` fills `:slug`, so the positional record falls through to the optional `:format` segment and yields `/posts/my-post.my-post`. Nested routes (`post_love_path`, `post_comments_path`) use `:post_slug`, so pass the record positionally there. Tests should assert against literal paths (`"/posts/#{post.slug}"`) rather than a route helper, or they can reproduce the very bug they check for.

### Content Export
Admins export the site from `/admin/exports` (`Admin::ExportsController`, admin role only; linked from the sidebar and `/admin/settings`). `Export` (`format` enum `markdown`/`json` with `validate: true`, so an unknown format is a validation error rather than an `ArgumentError`; `status` enum `pending`/`processing`/`completed`/`failed`; `has_one_attached :file`) keeps only the newest `Export::RETENTION_LIMIT` (10) rows, pruned `after_create_commit`. `ExportJob` calls `Export#exporter` (`Export.exporter_for(format)` → `Exports::MarkdownExporter` / `Exports::JsonExporter`, both in `app/services/exports/`, each `.call` returning a rewound `Tempfile` the caller must `close!`), attaches the result, and `broadcast_replace_to "exports"` with the `admin/exports/_export` partial so the index row updates live. Any exporter error marks the export `failed` with `error_message` instead of raising.

`Exports::MarkdownExporter` writes `posts/<slug>.md`, `pages/<slug>.md` (YAML front matter + body converted with the `reverse_markdown` gem) and `images/<blob id>-<slugified filename>` into a zip via `rubyzip` (streamed with `Zip::OutputStream`; blobs are downloaded in chunks). Rich text attachments are rewritten *before* HTML→Markdown conversion using `ActionText::Content#render_attachments`: image blobs become `../images/...` `<img>` tags, other blobs become links, and `XPost`/`YoutubeVideo` embeds become plain URL links. Filenames are slugified because a space in a Markdown link target breaks the link.

`Exports::JsonExporter` emits `{ format: "prose", version:, site:, authors:, categories:, tags:, posts:, pages:, navigation_items:, subscriber_labels:, subscribers: }` with rich text as HTML (`content_html`). **Site settings use the explicit `SITE_SETTING_ATTRIBUTES` allowlist**, not a denylist, so a newly added API key column is never exported by accident — add a column there only if it's safe to hand out. Password digests and subscriber auth tokens are never included.

Downloads go through `Admin::ExportDownloadsController` (`GET /admin/exports/:id/download`), which uses `ActiveStorage::Streaming#send_blob_stream` behind `require_admin` rather than redirecting to an Active Storage blob URL — those signed URLs don't expire, and a JSON export contains every subscriber email. It's a separate controller because `ActiveStorage::Streaming` includes `ActionController::Live`. `rubyzip` must stay a direct `Gemfile` dependency: it's otherwise only pulled in transitively by `selenium-webdriver` in the test group.

### Authentication
- **Admin**: session-based (signed cookie, 14-day expiry). The `Authentication` concern owns cookie → `Session` resumption (`resume_session`, memoized via a `Current.session` short-circuit) and is included once on `ApplicationController`, so both admin (`current_user`) and identity (`IdentityAuthentication#current_identity`) lookups share a single `sessions` query per request.
- **Admin Passkeys**: optional WebAuthn/passkey sign-in alongside password. Configured via `WEBAUTHN_ORIGIN` and `WEBAUTHN_RP_ID` env vars. Managed at `/admin/passkeys`.
- **Subscribers**: passwordless magic-link (15-minute token expiry)
- **MCP/API**: Bearer token (`prose_`-prefixed, SHA256 digest stored)

## Git Workflow

This project follows **GitHub Flow** (Feature Branch Workflow). All development happens on feature branches created from `master`. The `master` branch is always deployable.

### Rules

1. **Never commit directly to `master`.** Always create a feature branch.
2. **Branch from `master`** for every change — features, bug fixes, docs, refactors.
3. **Name branches descriptively**: `fix-typography-preview`, `add-subscriber-export`, `update-tailwind-config`.
4. **Before opening a pull request**, ensure:
   - All unit tests pass: `bin/rails test`
   - Linting passes with no offenses: `bin/rubocop`
   - Security scans are clean: `bin/brakeman --quiet --no-pager --exit-on-warn --exit-on-error`
5. **Submit a pull request** to merge back into `master`. PRs require review before merging.
6. **Keep PRs focused.** One logical change per PR — don't bundle unrelated work.

### Typical Workflow

```bash
git checkout master && git pull
git checkout -b my-feature-branch
# ... make changes ...
bin/rails test && bin/rubocop        # Verify before committing
git add <files> && git commit
git push -u origin my-feature-branch
gh pr create                          # Open pull request against master
```

## Code Style

- **Linter**: RuboCop with `rubocop-rails-omakase` preset (`.rubocop.yml`)
- **Testing**: Minitest with parallel execution; fixtures loaded from `test/fixtures/*.yml`
- Composition over inheritance; explicit dependencies over implicit magic

## Deployment

Docker + Kamal. SQLite databases persisted via volume mount (`prose_storage:/rails/storage`). Solid Queue runs in-process with Puma (`SOLID_QUEUE_IN_PUMA=true`). See the README for the full deployment guide.

**No credentials file** — the project does not use `config/credentials.yml.enc` or `config/master.key`. Instead:
- **Development/Test**: hardcoded Active Record Encryption keys in `config/environments/development.rb` and `test.rb`
- **Production**: all secrets come from ENV variables (`SECRET_KEY_BASE`, `ACTIVE_RECORD_ENCRYPTION_PRIMARY_KEY`, etc.)
- **Secret generation**: `bin/rails prose:generate_secrets` produces all required production secrets
- **Kamal secrets**: stored in `.kamal/.env` (gitignored), sourced by `.kamal/secrets`
