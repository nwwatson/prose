module AdminHelper
  ICONS = {
    ...
    "finger-print" => '<path stroke-linecap="round" stroke-linejoin="round" d="M7.864 4.243A7.5 7.5 0 0 1 19.5 10.5c0 2.92-.556 5.709-1.568 8.268M5.742 6.364A7.465 7.465 0 0 0 4.5 10.5a48.667 48.667 0 0 0 .048 6.062M12 10.5a2.25 2.25 0 0 0-4.5 0 48.493 48.493 0 0 0 .338 7.524M12 10.5V4.257c2.338.235 4.466 1.207 6.136 2.693M12 10.5c0 5.747-.414 11.332-1.176 16.636M7.629 18.875a48.7 48.7 0 0 1-.609-5.375M15.882 19.128A48.834 48.834 0 0 0 16.5 10.5c0-1.826-.653-3.5-1.738-4.8" />',
    ...
    "inbox" => '<path stroke-linecap="round" stroke-linejoin="round" d="m7.875 14.25 1.214 1.942a2.25 2.25 0 0 0 1.908 1.058h2.006c.776 0 1.497-.4 1.908-1.058l1.214-1.942M2.41 9h4.636a2.25 2.25 0 0 1 1.872 1.002l.164.246a2.25 2.25 0 0 0 1.872 1.002h2.092a2.25 2.25 0 0 0 1.872-1.002l.164-.246A2.25 2.25 0 0 1 16.954 9h4.636M2.41 9a2.25 2.25 0 0 0-.16.832V12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 12V9.832c0-.287-.055-.57-.16-.832M2.41 9a2.25 2.25 0 0 1 .382-.632l3.285-3.832a2.25 2.25 0 0 1 1.708-.786h8.43c.657 0 1.281.287 1.709.786l3.284 3.832c.163.19.291.404.382.632M4.5 20.25h15A2.25 2.25 0 0 0 21.75 18v-.75a2.25 2.25 0 0 0-2.25-2.25h-15a2.25 2.25 0 0 0-2.25 2.25V18a2.25 2.25 0 0 0 2.25 2.25Z" />',
  ...
  def render_trend(trend_data)
    return "" if trend_data.nil?
    change = trend_data[:change]
    return "" if change.zero? && trend_data[:previous].zero?
  ...
  def admin_nav_link(label, path, icon: nil, small: false)
    ...
    link_to path, class: "#{base_classes} #{active_classes}" do
      parts = []
      if icon && ICONS[icon]
        icon_classes = active ? "text-gray-500" : "text-gray-400 group-hover:text-gray-500"
        icon_size = small ? "mr-2 h-4 w-4" : "mr-3 h-5 w-5"
        parts << content_tag(:svg, ICONS[icon].html_safe, class: "flex-shrink-0 #{icon_size} #{icon_classes}",
